    ...
    public function add_action_links($links){
        ...
        */
        $settings_link = array(
            ...
            '<a href="https://bit.ly/2n6s0Hw" target="_blank" style="color:red;">' . __('BUY NOW', $this->plugin_name) . '</a>',
            );
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ays_show_results') {
            ...
            if(isset($results['user_email']) && $results['user_email'] !== ''){
                $row .= "<tr class=\"ays_result_element\">
                        ...
                        <td>".stripslashes($results['user_email'])."</td>
                        <td></td>
                        <td></td>
                     </tr>";
            }
            if(isset($results['user_name']) && $results['user_name'] !== ''){
                $row .= "<tr class=\"ays_result_element\">
                        ...
                        <td>".stripslashes($results['user_name'])."</td>
                        <td></td>
                        <td></td>
                     </tr>";
            }
            if(isset($results['user_phone']) && $results['user_phone'] !== ''){
                $row .= "<tr class=\"ays_result_element\">
                        ...
                        <td>".stripslashes($results['user_phone'])."</td>
                        <td></td>
                        <td></td>
                     </tr>";
            }
            ...
            if(isset($rate['score'])){
                $rate_html = '<tr style="vertical-align: top;" class="ays_result_element">
                    ...
                    <td style="max-width: 200px;">'. __("Review", $this->plugin_name).":<br>" . $rate['review'] . '</td>
                    <td></td>
                </tr>';
            }else{
                        ...
                        if($calc_method == 'by_correctness'){
                            if ($option == true) {
                                $row .= '<tr class="'.$tr_class.'">
                                            ...
                                            <td>'.__('Correct answer',$this->plugin_name).':<br/><p class="success">' . htmlentities(do_shortcode(stripslashes($correct_answers))) . '</p></td>
                                            <td>'.__('User answered',$this->plugin_name).':<br/><p class="success">' . htmlentities(do_shortcode(stripslashes($user_answered))) . '</p></td>
                                            <td>
                                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 130.2 130.2">
                                                    <circle class="path circle" fill="none" stroke="#73AF55" stroke-width="6" stroke-miterlimit="10" cx="65.1" cy="65.1" r="62.1"/>
                                                    <polyline class="path check" fill="none" stroke="#73AF55" stroke-width="6" stroke-linecap="round" stroke-miterlimit="10" points="100.2,40.2 51.5,88.8 29.8,67.5 "/>
                                                </svg>
                                                <p class="success">Succeed!</p>
                                            </td>
                                        </tr>';
                            } else {
                                $row .= '<tr class="'.$tr_class.'">
                                            <td>'.__('Question',$this->plugin_name).'' . $index . ' :<br/>' . (do_shortcode(stripslashes($question["question"]))) . '</td>
                                            <td>'.__('Correct answer',$this->plugin_name).':<br/><p class="success">' . htmlentities(do_shortcode(stripslashes($correct_answers))) . '</p></td>
                                            <td>'.__('User answered',$this->plugin_name).':<br/><p class="error">' . htmlentities(do_shortcode(stripslashes($user_answered))) . '</p></td>
                                            <td>
                                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 130.2 130.2">
        ...
        if(isset($_REQUEST['page'])){
            if(false !== strpos($_REQUEST['page'], $this->plugin_name)){
                ...
                <p style="font-size:13px;text-align:center;font-style:italic;">
                    <span><?php echo __( "If you love our plugin, please do big favor and rate us on", $this->plugin_name); ?></span> <a target="_blank" href='https://bit.ly/2m4Cya8'>WordPress.org</a>
                </p>
            <?php
            }
        }
