...
class Quiz_Maker_Public
{
    ...
    /*******************************************************************************************************/
        ...
        $form_title = (isset($options['form_title']) && $options['form_title'] != '') ? do_shortcode(stripslashes(wpautop($options['form_title']))): '';
    ...
    /*******************************************************************************************************/
        ...
        if (isset($options['enable_timer']) && $options['enable_timer'] == 'on') {
            ...
            $timer_text = do_shortcode(stripslashes(str_replace('%%time%%', $this->secondsToWords($options['timer']), wpautop($timer_text))));
            $hide_timer_cont = "";
            if($timer_text == ""){
    ...
    /*******************************************************************************************************/
        ...
        if(isset($options['rate_form_title'])){
            $rate_form_title = do_shortcode(stripslashes(wpautop($options['rate_form_title'])));
        }
    ...
    /*******************************************************************************************************/
        ...
        if (isset($options['limit_users']) && $options['limit_users'] == "on") {
            ...
            if ($result != 0) {
                ...
                $limit_users_html = $timer_row . "<div style='color:" . $text_color . "' class='ays_block_content'>" . do_shortcode(stripslashes(wpautop($options['limitation_message']))) . "</div><style>form{min-height:0 !important;}</style>";
            }
        }else{
            $limit_users = false;
        }
    ...
    /*******************************************************************************************************/
        ...
        $quiz_styles .= "}
            ...
            #ays-quiz-container-" . $id . " .ays-start-page *,
    ...
    public function ays_finish_quiz(){
        ...
        if (isset($_REQUEST["ays_finish_quiz"]) && wp_verify_nonce($_REQUEST["ays_finish_quiz"], 'ays_finish_quiz')) {
            ...
            if (is_array($questions_answers)) {
                ...
                $result_text = (isset($options->result_text) && $options->result_text != '') ? do_shortcode(stripslashes(wpautop($options->result_text))) : '';
